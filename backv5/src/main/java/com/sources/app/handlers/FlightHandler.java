 ...
 * 
 * <p>Este handler implementa HttpHandler de com.sun.net.httpserver para
 * procesar solicitudes HTTP y retornar respuestas en formato JSON.</p>
 * 
 * <p>Endpoints soportados:</p>
 * <ul>
 *   <li>GET /api/airline/flights - Listar todos los vuelos</li>
 *   <li>GET /api/airline/flights/{id} - Obtener detalles de un vuelo</li>
 *   <li>GET /api/airline/flights/{id}/seats - Obtener asientos disponibles</li>
 *   <li>POST /api/airline/flights - Crear nuevo vuelo</li>
 *   <li>POST /api/airline/flights/bulk - Crear múltiples vuelos</li>
 *   <li>POST /api/airline/flights/{id}/stopover - Agregar escala a un vuelo</li>
 *   <li>PUT /api/airline/flights/{id} - Actualizar un vuelo</li>
 * </ul>
 * 
 * <p>Características:</p>
 * <ul>
 ...
 *   <li>Gestión de errores con mensajes descriptivos</li>
 *   <li>Integración con Hibernate para persistencia</li>
 *   <li>Búsqueda avanzada de vuelos con filtros</li>
 * </ul>
 * 
...
public class FlightHandler implements HttpHandler {
    /** DAO para operaciones de persistencia de vuelos */
    private final FlightDAO flightDAO;
    
    /** Librería Gson para serialización/deserialización JSON */
    private final Gson gson;
    
    /**
     * Constructor por defecto que inicializa el handler.
     * Crea instancias de FlightDAO y Gson necesarias para el funcionamiento.
     */
    ...
    /**
     * Maneja las solicitudes HTTP entrantes para operaciones de vuelos.
     * Este método es el punto de entrada principal para todas las requests
     * relacionadas con vuelos.
     * 
     * <p>Procesa solicitudes según el método HTTP y la ruta:</p>
     * <ul>
     *   <li>OPTIONS: Retorna headers CORS sin contenido</li>
     *   <li>GET: Obtiene vuelos o información específica</li>
     *   <li>POST: Crea nuevos vuelos o agrega escalas</li>
     *   <li>PUT: Actualiza vuelos existentes</li>
     * </ul>
     * 
     * @param exchange El objeto HttpExchange que contiene request y response
     * @throws IOException Si ocurre un error al leer/escribir en el stream
     */
    @Override
    ...
    private String getFlightsWithOptionalFilters(HttpExchange exchange) {
        try {
            ...
            // 🔍 BÚSQUEDA CON ESCALA: Siempre buscar vuelos con 1 escala si se especificaron origen y destino
            List<Map<String, Object>> oneStopFlights = new ArrayList<>();
            if (origin != null && dest != null) {
                System.out.println("🔍 Buscando vuelos con 1 escala de " + origin + " a " + dest + "...");
                oneStopFlights = searchOneStopFlights(origin, dest);
                System.out.println("✈️ Vuelos con escala encontrados: " + oneStopFlights.size());
            }
            
            // 🔄 BÚSQUEDA ROUND-TRIP: Si se especificó returnDate, buscar vuelos de vuelta
            List<Map<String, Object>> returnFlights = new ArrayList<>();
            if (retDate != null && !retDate.isBlank() && origin != null && dest != null) {
                System.out.println("🔄 Buscando vuelos de vuelta para round-trip...");
                // Buscar vuelos en dirección inversa
                List<Flight> returnsFound = flightDAO.searchFlights(dest, origin, retDate, null, pax, minPrice, maxPrice, seatCat, nonstop);
                for (Flight rf : returnsFound) {
                    Map<String, Object> rfMap = new HashMap<>();
                    rfMap.put("idFlight", rf.getIdFlight());
                    rfMap.put("flightNumber", rf.getFlightNumber());
                    rfMap.put("originCity", rf.getOriginCity() != null ? rf.getOriginCity().getName() : "N/A");
                    rfMap.put("destinationCity", rf.getDestinationCity() != null ? rf.getDestinationCity().getName() : "N/A");
                    rfMap.put("departureDate", rf.getDepartureDate());
                    rfMap.put("departureTime", rf.getDepartureTime());
                    rfMap.put("arrivalDate", rf.getArrivalDate());
                    rfMap.put("arrivalTime", rf.getArrivalTime());
                    rfMap.put("basePrice", rf.getBasePrice());
                    rfMap.put("type", "RETURN");
                    returnFlights.add(rfMap);
                }
                System.out.println("🔄 Vuelos de vuelta encontrados: " + returnFlights.size());
            }
            ...
            response.put("success", true);
            response.put("flights", flightMaps);
            response.put("oneStopFlights", oneStopFlights);  // Vuelos con 1 escala
            response.put("returnFlights", returnFlights);     // Vuelos de vuelta (round-trip)
            response.put("hasOneStop", !oneStopFlights.isEmpty());
            response.put("hasReturn", !returnFlights.isEmpty());
            
            System.out.println("📊 Respuesta: " + flightMaps.size() + " directos, " + oneStopFlights.size() + " con escala, " + returnFlights.size() + " vuelta");
    ...
    /**
     * Busca rutas con exactamente 1 escala entre origen y destino.
     * Retorna combinaciones de 2 vuelos que conectan.
     */
    private List<Map<String, Object>> searchOneStopFlights(Integer originId, Integer destId) {
        List<Map<String, Object>> oneStopRoutes = new ArrayList<>();
        
        try {
            List<Flight> allFlights = flightDAO.getAllFlights();
            
            // Filtrar solo vuelos programados
            List<Flight> scheduled = new ArrayList<>();
            for (Flight f : allFlights) {
                if ("SCHEDULED".equals(f.getStatus()) && f.getOriginCity() != null && f.getDestinationCity() != null) {
                    scheduled.add(f);
                }
            }
            
            // Buscar combinaciones: vuelo1 (origen → escala) + vuelo2 (escala → destino)
            for (Flight f1 : scheduled) {
                if (!f1.getOriginCity().getIdCity().equals(originId.longValue())) continue;
                
                Long viaCityId = f1.getDestinationCity().getIdCity();
                
                for (Flight f2 : scheduled) {
                    if (f2.getOriginCity().getIdCity().equals(viaCityId) && 
                        f2.getDestinationCity().getIdCity().equals(destId.longValue())) {
                        
                        Map<String, Object> route = new HashMap<>();
                        route.put("type", "ONE_STOP");
                        route.put("viaCityId", viaCityId);
                        route.put("viaCityName", f1.getDestinationCity().getName());
                        
                        // Primer segmento (origen → escala)
                        Map<String, Object> seg1 = new HashMap<>();
                        seg1.put("idFlight", f1.getIdFlight());
                        seg1.put("flightNumber", f1.getFlightNumber());
                        seg1.put("originCity", f1.getOriginCity().getName());
                        seg1.put("destinationCity", f1.getDestinationCity().getName());
                        seg1.put("departureDate", f1.getDepartureDate());
                        seg1.put("departureTime", f1.getDepartureTime());
                        seg1.put("arrivalDate", f1.getArrivalDate());
                        seg1.put("arrivalTime", f1.getArrivalTime());
                        seg1.put("basePrice", f1.getBasePrice());
                        
                        // Segundo segmento (escala → destino)
                        Map<String, Object> seg2 = new HashMap<>();
                        seg2.put("idFlight", f2.getIdFlight());
                        seg2.put("flightNumber", f2.getFlightNumber());
                        seg2.put("originCity", f2.getOriginCity().getName());
                        seg2.put("destinationCity", f2.getDestinationCity().getName());
                        seg2.put("departureDate", f2.getDepartureDate());
                        seg2.put("departureTime", f2.getDepartureTime());
                        seg2.put("arrivalDate", f2.getArrivalDate());
                        seg2.put("arrivalTime", f2.getArrivalTime());
                        seg2.put("basePrice", f2.getBasePrice());
                        
                        route.put("firstSegment", seg1);
                        route.put("secondSegment", seg2);
                        route.put("totalPrice", f1.getBasePrice().add(f2.getBasePrice()));
                        
                        oneStopRoutes.add(route);
                    }
                }
            }
        } catch (Exception e) {
            System.err.println("❌ Error buscando vuelos con escala: " + e.getMessage());
        }
