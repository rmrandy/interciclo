...
public class TicketDAO {
    ...
    /**
     * Obtiene todos los tickets comprados por un usuario empresarial.
     * Útil para que la agencia vea todas las compras que ha realizado.
     * 
     * @param corporateUserId ID del usuario empresarial (agencia)
     * @return Lista de tickets comprados por la agencia
     */
    public List<Ticket> getTicketsByPurchasedByUserId(Integer corporateUserId) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            String hql = "FROM Ticket t WHERE t.purchasedByUser.idUser = :corporateUserId ORDER BY t.createdAt DESC";
            List<Ticket> tickets = session.createQuery(hql, Ticket.class)
                    .setParameter("corporateUserId", corporateUserId)
                    .getResultList();
            System.out.println("🏢 Tickets empresariales encontrados: " + tickets.size() + " para usuario " + corporateUserId);
            return tickets;
        } catch (Exception e) {
            System.err.println("❌ Error obteniendo tickets empresariales: " + e.getMessage());
            e.printStackTrace();
    ...
    public Ticket createTicketWithValidation(Map<String, Object> ticketData) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            // 🏢 SOPORTE EMPRESARIAL: Si viene purchasedByUserId, es una compra empresarial
            if (ticketData.containsKey("purchasedByUserId") && ticketData.get("purchasedByUserId") != null) {
                Integer purchasedByUserId = (Integer) ticketData.get("purchasedByUserId");
                User purchasedByUser = session.get(User.class, purchasedByUserId);
                if (purchasedByUser != null && purchasedByUser.isCorporateUser()) {
                    ticket.setPurchasedByUser(purchasedByUser);
                    System.out.println("🏢 Ticket comprado por usuario empresarial: " + purchasedByUser.getCompanyName());
                }
            }
