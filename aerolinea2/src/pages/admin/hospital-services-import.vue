...
async function tryMultipleIPs(endpoint: string, method: string = 'GET', data: any = null) {
  ...
  for (const serverIP of possibleIPs) {
    try {
      console.log(`Intentando ${method} a http://${serverIP}:2020/api${endpoint}`);
      const url = `http://${serverIP}:2020/api${endpoint}`;
...
const testConnection = async () => {
  ...
  for (const ip of possibleIPs) {
    try {
      ...
      await axios.get(`http://${ip}:2020/api/healthcheck`, {
        timeout: 3000
      });
      results[ip] = true;
    } catch (err) {
      try {
        ...
        await axios.get(`http://${ip}:2020/api/users/count`, {
          timeout: 3000
        });
        results[ip] = true;
      } catch (err2) {
    ...
    <div v-if="error" class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4" role="alert">
      ...
      <div class="mt-3 text-sm">
        ...
        <ul class="list-disc pl-5 mt-1">
          ...
          <li>URL de API del seguro: http://[servidor]:2020/api</li>
          ...
          <div v-if="Object.keys(connectionTestResults).length > 0" class="mt-2 p-2 bg-gray-50 rounded">
            ...
            <ul class="mt-1">
              <li v-for="(result, ip) in connectionTestResults" :key="ip" class="flex items-center">
                ...
                <span>{{ ip }}:2020 - {{ result ? 'Conectado' : 'No disponible' }}</span>
              </li>
            </ul>
          </div>
        ...
        <div class="mt-3">
          <p>Sugerencias:</p>
          <ul class="list-disc pl-5 mt-1">
            <li>Verifique que el servidor backend esté ejecutándose en el puerto 2020</li>
