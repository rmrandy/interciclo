...
- **URL Base**: `http://localhost:2020/api/user`
- **Puerto**: 2020
...
mvn exec:java -Dexec.mainClass="com.sources.app.UserProfileServer" -Dexec.args="2020"
```

### **3. Verificar que esté Funcionando**
```bash
curl http://localhost:2020/api/user/test
```
...
```bash
# Obtener perfil
curl -X POST http://localhost:2020/api/user/profile \
  -H "Content-Type: application/json" \
  -d '{"userId": 1}'

# Obtener configuraciones
curl -X POST http://localhost:2020/api/user/settings \
  -H "Content-Type: application/json" \
  -d '{"userId": 1}'
```
...
- Verificar que el puerto 2020 esté libre
