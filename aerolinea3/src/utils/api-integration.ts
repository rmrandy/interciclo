...
const INSURANCE_API_URL = `http://${INSURANCE_IP}:6060/api`;
...
const getApprovedHospitals = async (): Promise<Hospital[]> => {
  try {
    const response = await axios.get(`http://${INSURANCE_IP}:6060/api/hospital`);
    return response.data;
  } catch (error) {
...
const getHospitalServices = async (hospitalId?: number, hospitalPort?: string): Promise<HospitalService[]> => {
  try {
    ...
    const response = await axios.get(
      `http://${INSURANCE_IP}:6060/api/hospital-proxy/${hospitalId}/api/services/`,
      { headers }
    );
    ...
    try {
      const alternativeResponse = await axios.get(
        `http://${INSURANCE_IP}:6060/api/hospital-proxy/${hospitalId}/services/`,
        { headers }
      );
...
const getHospitalCategories = async (hospitalId?: number, hospitalPort?: string): Promise<Category[]> => {
  try {
    ...
    // Usar el nuevo proxy que maneja el puerto automáticamente
    const response = await axios.get(
      `http://${INSURANCE_IP}:6060/api/hospital-proxy/${hospitalId}/api/categories/`,
      { headers }
    );
...
const syncHospitalServices = async (hospitalId: number, insuranceId: string, hospitalPort?: string) => {
  try {
    ...
    const response = await axios.post(
      `http://${INSURANCE_IP}:6060/api/hospital-proxy/${hospitalId}/api/services_ensurance/import/`,
...
const testCorsHospital = async (hospitalId: number, hospitalPort?: string): Promise<any> => {
  try {
    ...
    // Usar el nuevo proxy que maneja el puerto automáticamente
    const response = await axios.get(
      `http://${INSURANCE_IP}:6060/api/hospital-proxy/${hospitalId}/cors-test/`,
      { headers }
    );
