...
function addToCartLocal(product) {
  ...
  console.log('DEBUG usuario actual (internacional):', user);
  if (user && user.idUser) {
    mensaje.value = 'Funcionalidad internacional: aquí deberías agregar al backend real internacional.';
    // Aquí iría la lógica real si tuvieras backend internacional
    return;
  }
  ...
  mensaje.value = 'Debes iniciar sesión para agregar productos al carrito internacional real. Producto agregado al carrito local.';
  eventBus.emit('carrito-actualizado');
  // Log de depuración
  console.log('Agregado al carrito internacional local:', { key, port, cart });
}
</script>
